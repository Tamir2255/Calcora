...
# ── Diagnostics helper ────────────────────────────────────────────────────────
# Centralised, non-fatal error logging. Keeps the app's offline-first / never-
# crash behaviour, but makes failures visible in the console instead of being
# silently swallowed by bare "except: pass" blocks.

def log_error(context, exc):
    print(f"[Calcora] {context}: {exc}")
...
def load_stores():
    ...
    try:
        ...
        log_error("load_stores failed", e)
...
def save_stores(data):
    try:
        ...
        log_error("save_stores failed", e)
...
def apply_theme(mode):
    ...
    try:
        ...
        log_error("apply_theme failed to set window color", e)
...
def get_biz_type():
    ...
    try:
        ...
        log_error("get_biz_type failed", e)
    return "Home Retail"
...
def app_currency():
    ...
    try:
        ...
        log_error("app_currency failed", e)
    return "UGX"
...
class StockScreen(CalcoraScreen):
    ...
    def on_pre_enter(self, *_):
        # Stock is PIN-protected like Finance/Profile: require the PIN again
        # each time the screen is entered, rather than staying unlocked
        # forever after the very first build (previous behaviour effectively
        # never re-locked Stock at all).
        self.clear_widgets()
        self._unlocked = False
        self._build_locked()
    ...
    def _do_build(self):
        ...
        # Lock button row
        lock_row = BoxLayout(size_hint_y=None, height=dp(30))
        lock_row.add_widget(Widget())
        lock_row_btn = CalcoraButton(text="Lock", background_color=C_PANEL2,
                                      size_hint=(None, None),
                                      size=(dp(70), dp(28)), font_size=dp(11))
        lock_row_btn.bind(on_release=self._lock)
        lock_row.add_widget(lock_row_btn)
        page.add_widget(lock_row)
    ...
    def refresh(self):
        # on_pre_enter already handles the re-lock; this only rebuilds
        # content for an already-unlocked, mid-session refresh.
        if self._unlocked:
            self.clear_widgets()
            self._do_build()
    ...
    def _save(self, require_existing):
        ...
        # Snapshot prior state so we can detect real threshold *crossings*
        # instead of re-notifying on every single save regardless of
        # whether the stock situation actually changed.
        prev_qty         = item["quantity"]
        prev_total_added = item["total_quantity_added"]
        prev_threshold    = max(1, prev_total_added * 0.25)
        was_low           = exists and prev_qty <= prev_threshold
        was_out           = exists and prev_qty <= 0
        ...
        # Keep the weighted-average cost as the authoritative buying price
        # used for profit/tax math elsewhere (see SalesScreen._submit).
        # Previously this field was overwritten with just the latest
        # restock's entered price, which skewed profit away from the true
        # blended cost basis every time an item was restocked at a
        # different price.
        item["buying_price"]  = item["unit_pb"] if item["unit_pb"] else buying_price
        ...
        # Stock notifications: only fire on real threshold *crossings*, not
        # on every save. Previously "Stock over"/"Low stock" fired on every
        # single Add/Restock regardless of whether anything meaningful
        # changed, flooding the owner's notification feed.
        store_name = ""
        try:
            with open(app.active_profile_file(), "r", encoding="utf-8") as f:
                store_name = json.load(f).get("store_name", "")
        except Exception as e:
            log_error("Could not read store_name for notification", e)
        if not store_name:
            store_name = "Your Store"

        new_threshold = max(1, item["total_quantity_added"] * 0.25)
        new_low       = item["quantity"] <= new_threshold

        if was_out and item["quantity"] > 0:
            # Item was out of stock and has just been replenished.
            send_app_notification(app.active_store_id,
                                  _format_notification_text(
                                      "Restocked", name, store_name))
        if new_low and not was_low:
            # Just crossed *into* the low-stock zone.
            send_app_notification(app.active_store_id,
                                  _format_notification_text(
                                      "Low stock", name, store_name))
        elif was_low and not new_low:
            # Recovered out of the low-stock zone.
            send_app_notification(app.active_store_id,
                                  _format_notification_text(
                                      "Stock replenished above threshold",
                                      name, store_name))
...
class SalesScreen(CalcoraScreen):
    ...
    def _submit(self, *_):
        ...
        for row in rows:
            ...
            # Prefer the weighted-average cost (unit_pb) as the true cost
            # basis for profit/tax math; fall back to buying_price only if
            # unit_pb hasn't been set for some reason. Previously this
            # preferred the latest-entered buying_price over the proper
            # running average, understating/overstating profit after any
            # restock at a different price.
            bp                  = si.get("unit_pb") or si.get("buying_price", 0)
            row["buying_price"] = bp
...
#
# PINs and owner passwords are hashed with salted PBKDF2-HMAC-SHA256. For
# backward compatibility with values saved by earlier versions of the app
# (a bare, unsalted SHA-256 hash), verification falls back to the legacy
# check when the stored value doesn't look like the new "<salt>$<hash>"
# format. Anything *set* from now on uses the new, stronger format, so
# existing installs upgrade automatically the next time a PIN/password is
# changed.

_PBKDF2_ITERATIONS = 200_000

def _legacy_hash(text):
    """Old, unsalted SHA-256 hash — kept only to verify pre-existing values."""
...
def verify_pin_hash(pin, stored):
    """Verify a PIN/password against a stored hash (new or legacy format)."""
    if not stored:
        return False
    if "$" in stored:
        salt_hex, _ = stored.split("$", 1)
        return hash_pin(pin, salt_hex) == stored
    # Legacy unsalted SHA-256 hash
    return _legacy_hash(pin) == stored
...
def load_pin():
    ...
    try:
        ...
        log_error("load_pin failed", e)
    return ""


# Lightweight in-memory lockout to slow down brute-force PIN guessing. A
# 4-digit PIN only has 10,000 possibilities, so an attacker hammering the
# "Enter PIN" popup could exhaust it quickly; this adds a short cooldown
# after repeated failures. Deliberately simple (per-process, resets on
# restart) — it isn't a substitute for the worker-login hardening in
# supabase_sync.py, since worker login doesn't go through this popup.
_pin_fail_counts    = {}
_PIN_MAX_ATTEMPTS   = 5
_PIN_LOCKOUT_SECONDS = 30

def _pin_lockout_key():
    app = App.get_running_app()
    return app.active_pin_file() if app else PIN_FILE

def _pin_locked_out():
    key = _pin_lockout_key()
    fails, until = _pin_fail_counts.get(key, (0, 0))
    if fails >= _PIN_MAX_ATTEMPTS:
        if time.time() < until:
            return True, until - time.time()
        # Lockout window has expired — clear the counter so the next
        # attempt starts fresh. (Only reached once fails has actually hit
        # the threshold; below-threshold counts must NOT be cleared here,
        # since until==0 for them and time.time() >= 0 is always true —
        # that earlier version reset the counter on every single check,
        # before it could ever reach the threshold.)
        _pin_fail_counts.pop(key, None)
    return False, 0

def _pin_record_failure():
    key = _pin_lockout_key()
    fails, _ = _pin_fail_counts.get(key, (0, 0))
    fails += 1
    until = time.time() + _PIN_LOCKOUT_SECONDS if fails >= _PIN_MAX_ATTEMPTS else 0
    _pin_fail_counts[key] = (fails, until)

def _pin_record_success():
    _pin_fail_counts.pop(_pin_lockout_key(), None)

def check_pin(pin):
    stored = load_pin()
    return bool(stored) and verify_pin_hash(pin, stored)
...
def pin_popup(title, on_success, on_cancel=None, confirm=False, worker_mode=False):
    ...
    # If this is a normal PIN-check (not a set-up/confirm flow and not the
    # worker's freeform-entry mode), show any existing lockout up front.
    if not worker_mode and not confirm:
        locked, remaining = _pin_locked_out()
        if locked:
            msg.text  = f"Too many attempts. Try again in {int(remaining)+1}s."
            msg.color = C_ERROR
...
# ── Auth helpers ──────────────────────────────────────────────────────────────
#
# Owner passwords use the same salted PBKDF2 scheme as PINs (see hash_pin /
# verify_pin_hash above), with a legacy fallback for accounts created before
# this change.

def hash_pw(password):
    """Salted PBKDF2 hash for a new/changed password."""
    return hash_pin(password)

def verify_pw(password, stored):
    """Verify a password against a stored hash (new or legacy format)."""
    return verify_pin_hash(password, stored)
...
class SignupScreen(Screen):
    ...
    def _finish(self, business, store_name, email, phone, country, cur, pw):
        ...
        try:
            ...
            log_error("Could not write initial profile", e)
...
class LoginScreen(Screen):
    ...
    def _finish(self, email, pw, fb_ok, fb_msg):
        ...
        if not verify_pw(pw, auth.get("password","")):
    ...
    def _build(self):
        ...
        def _load_summary():
            try:
                ...
                w_count = len(workers)
                # Compute health with the single shared implementation in
                # health_utils, instead of a separately-maintained inline
                # copy (previously reimplemented here, in the per-store
                # stats block below, and again in the weekly notification —
                # each could silently drift from the others).
                from datetime import datetime, timedelta
                from health_utils import compute_base_health
                week_ago = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")
                recent = [x for x in sales if (x and (x.get("submitted_at") or x.get("created_at") or "")[:10]) >= week_ago]
                health = compute_base_health(recent)
                def _upd(dt):
                ...
                log_error("OwnerDashboard summary load failed", e)
        ...
        def _check_online():
            ...
            if not online and FB_AVAILABLE and fb:
                try:
                    ...
                    log_error("refresh_session failed", e)
            def _upd(dt):
                if online:
    ...
    def _wait_for_supabase(self, seconds=5):
        ...
        while time.time() < end:
            ...
            try:
                ...
                log_error("_wait_for_supabase refresh failed", e)
            time.sleep(0.25)
        return fb.is_online()
            ...
            def _show(dt):
                ...
                for s in stores:
                    ...
                    def _stats(store_id, store_name, revenue_l, sales_l, worker_l, hb, hp, wl, lw_lbl):
                        ...
                        # Single shared health_utils call (previously
                        # reimplemented here and could drift from the
                        # version used in the weekly notification).
                        health   = compute_base_health(recent)
                        ...
                        def _upd(dt):
                            ...
                            try:
                                ...
                                change_pct = compute_change_pct(cur_total, prev_total)
                            except Exception as e:
                                log_error("weekly change calc failed", e)
                            ...
                            try:
                                ...
                                log_error("last-week label update failed", e)

                            # Adjusted health notification (cap +/-15 points).
                            # Same shared health_utils call used everywhere
                            # else, so this can't drift from the weekly
                            # notification's number anymore.
                            adj_h = adjust_health(health, change_pct, cap_points=15)
                            if adj_h < 15:
...
class ProfileScreen(CalcoraScreen):
    ...
    def _load_profile(self):
        ...
        try:
            ...
            log_error("Profile._load_profile failed", e)
        ...
        try:
            ...
            if os.path.exists(pf):
                try:
                    ...
                    log_error("Profile._save_profile: reading existing profile failed", e)
                    existing = {}
            ...
            log_error("Profile._save_profile failed to write", e)
    ...
    def _flash(self, text):
        ...
        try:
            ...
            log_error("Profile._flash failed", e)
...
class CalcoraApp(App):
    ...
    def switch_store(self, store_id):
        ...
        for sname in ("home","sales","stock","finance","details","history","profile"):
            try:
                ...
                log_error(f"switch_store: refresh of screen '{sname}' failed", e)
    ...
    def _get_sales_for_store(self, store_id, limit=1000):
        ...
        if FB_AVAILABLE and fb and fb.is_online():
            try:
                ...
                log_error("_get_sales_for_store: fb.get_sales failed", e)
                sales = []
        if not sales:
    ...
    def on_start(self):
        ...
        try:
            ...
            log_error("on_start: setting window icon failed", e)
        ...
        try:
            ...
            log_error("on_start: scheduling weekly health check failed", e)
    ...
    def _check_subscription_notifications(self):
        ...
        try:
            ...
            log_error("_check_subscription_notifications: read failed", e)
            return
        profile = _ensure_subscription_fields(profile)
        ...
        try:
            ...
            try:
                ...
                log_error("_check_subscription_notifications: bad expiry date", e)
                return
        ...
        try:
            ...
            log_error("_check_subscription_notifications: write failed", e)
    ...
    def _weekly_health_check(self):
        ...
        try:
            ...
            log_error("_weekly_health_check: read profile failed", e)
            return
        ...
        if last_reported:
            try:
                ...
                log_error("_weekly_health_check: bad last_week_reported", e)
        ...
        # Use the single shared health_utils implementation for both the
        # percent-change calculation and the capped health adjustment, so
        # this notification's numbers can never drift from the dashboard's
        # (previously this block reimplemented the same math inline,
        # without applying the +/-15 point cap that health_utils.adjust_health
        # enforces elsewhere).
        from health_utils import compute_base_health, compute_change_pct, adjust_health
        change_pct      = compute_change_pct(current_total, previous_total)
        base_health     = compute_base_health(weekly_sales)
        adjusted_health = adjust_health(base_health, change_pct, cap_points=15)
        ...
        try:
            ...
            log_error("_weekly_health_check: write failed", e)
    ...
    def vat_rate(self):
        """Read VAT % from the ACTIVE store's profile, fallback to 18%.

        Previously this always read the default-store PROFILE_FILE, so a
        non-default active store's own VAT setting was silently ignored in
        all profit/tax math. Now mirrors active_profile_file()/get_biz_type().
        """
        try:
            pf = self.active_profile_file()
            if os.path.exists(pf):
            ...
            log_error("vat_rate failed", e)
        return VAT_RATE

    def currency(self):
        """Read currency from the ACTIVE store's profile, fallback to UGX.

        Previously this always read the default-store PROFILE_FILE, so a
        non-default active store's own currency was silently ignored.
        """
        try:
            pf = self.active_profile_file()
            if os.path.exists(pf):
