...
# ── Config ────────────────────────────────────────────────────────────────────
#
# Prefer environment variables so real deployments don't have to ship
# credentials in source control; the literals below remain as a fallback so
# this still runs out-of-the-box during development. The anon key is
# designed to be public-ish (it only grants what Supabase Row Level
# Security policies allow), but moving it out of source is still good
# hygiene and makes per-environment overrides (staging vs prod) trivial.

SUPABASE_URL = os.environ.get("CALCORA_SUPABASE_URL",
    "https://hcnjxdidapkkyrbxxtdy.supabase.co")
ANON_KEY     = os.environ.get("CALCORA_SUPABASE_ANON_KEY", (
...
def _hash(text):
    """Legacy unsalted SHA-256 hash — kept only to verify PINs that were
    stored by older versions of the app. New PINs use _hash_pin below."""
    return hashlib.sha256(text.encode()).hexdigest()

_PBKDF2_ITERATIONS = 200_000

def _hash_pin(pin, salt=None):
    """Salted PBKDF2-HMAC-SHA256 hash, stored as '<salt_hex>$<hash_hex>'.

    A bare 4-digit PIN only has 10,000 possibilities, so an unsalted,
    single-round SHA-256 hash (the previous scheme) is brute-forceable in a
    fraction of a second if it ever leaks — PBKDF2 with a high iteration
    count and a per-PIN salt makes that far more expensive, and the salt
    means two workers who happen to pick the same PIN don't produce the
    same stored hash.
    """
    if salt is None:
        salt = os.urandom(16)
...
def _verify_pin(pin, stored):
    """Verify a PIN against a stored hash, supporting the new salted format
    and falling back to the legacy unsalted format for PINs set before this
    change."""
    if not stored:
        return False
    if "$" in stored:
        salt_hex, _ = stored.split("$", 1)
        return _hash_pin(pin, salt_hex) == stored
...
# ── Worker login rate limiting ────────────────────────────────────────────────
#
# worker_login() below authenticates by fetching every worker row for a
# store (using only the public anon key — there's no per-worker Supabase
# Auth account) and comparing PIN hashes client-side. That means:
#   1. Anyone holding the anon key can list a store's workers if Row Level
#      Security on the `workers` table doesn't lock reads down tightly, and
#   2. The PIN check itself has no server-side attempt limiting, so a 4-digit
#      PIN (10,000 possibilities) could be brute-forced by scripting repeated
#      calls to this function.
# A proper fix moves PIN verification into a Postgres RPC function
# (SECURITY DEFINER) that takes (store_code, pin) and returns only a
# worker id/name on success, never exposing pin_hash to the client at all —
# that also lets Supabase-side rate limiting/logging apply. That schema
# change is outside what this client module can enforce on its own, so as
# a client-side mitigation in the meantime, this lockout at least slows
# down brute-force attempts made through this app.
_worker_login_fail_counts     = {}
_WORKER_LOGIN_MAX_ATTEMPTS    = 5
_WORKER_LOGIN_LOCKOUT_SECONDS = 30

def _worker_login_locked_out(store_code):
    key = store_code.upper()
    fails, until = _worker_login_fail_counts.get(key, (0, 0))
    if fails >= _WORKER_LOGIN_MAX_ATTEMPTS:
        if time.time() < until:
        ...
        _worker_login_fail_counts.pop(key, None)
    return False, 0

def _worker_login_record_failure(store_code):
    key = store_code.upper()
    fails, _ = _worker_login_fail_counts.get(key, (0, 0))
    fails += 1
    until = (time.time() + _WORKER_LOGIN_LOCKOUT_SECONDS
             if fails >= _WORKER_LOGIN_MAX_ATTEMPTS else 0)
    _worker_login_fail_counts[key] = (fails, until)

def _worker_login_record_success(store_code):
    _worker_login_fail_counts.pop(store_code.upper(), None)


def worker_login(store_code, worker_pin):
    """Worker logs in with store code + PIN — no Supabase Auth needed."""
    locked, remaining = _worker_login_locked_out(store_code)
    if locked:
        return (False,
                f"Too many attempts. Try again in {int(remaining)+1}s.",
                None, None)
    try:
        ...
        for w in workers:
            if _verify_pin(worker_pin, w.get("pin_hash", "")):
                ...
                _worker_login_record_success(store_code)
                return True, "Logged in", store, w

        _worker_login_record_failure(store_code)
